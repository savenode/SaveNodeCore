...
#if defined(NDEBUG)
#error "savenode cannot be compiled without assertions."
#endif
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      if (nHeight == 0) {
             nSubsidy = 300000 * COIN;
                ...
                nSubsidy = 1 * COIN;
      } else if (nHeight <= 15000 && nHeight > 100) {
          nSubsidy = 5 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from savenode
...
{
    RenameThread("savenode-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZsavenodeMinted()
{
...
void RecalculateZsavenodeSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
...
bool RecalculatesavenodeSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert savenode to zsnd
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // savenode: We use certain sporks during IBD, so check to see if they are
