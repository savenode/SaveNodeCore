...
#include "zsavenodecontroldialog.h"
#include "ui_zsavenodecontroldialog.h"
...
std::list<std::string> ZsavenodeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZsavenodeControlDialog::listMints;

ZsavenodeControlDialog::ZsavenodeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZsavenodeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZsavenodeControlDialog::~ZsavenodeControlDialog()
{
    delete ui;
}

void ZsavenodeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZsavenodeControlDialog::updateList()
{
...
void ZsavenodeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZsavenodeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZsavenode_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZsavenodeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZsavenodeControlDialog::GetSelectedMints()
{
...
void ZsavenodeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
