...
void PrivacyDialog::on_pushButtonZsavenodeControl_clicked()
{
    ZsavenodeControlDialog* zsndControl = new ZsavenodeControlDialog(this);
    zsndControl->setModel(walletModel);
    zsndControl->exec();
}

void PrivacyDialog::setZsavenodeControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzsnd()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid savenode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " savenode </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZsavenodeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZsavenodeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsavenode selector in case it was used
    ZsavenodeControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " savenode, ";
